...
def receiver():
    with SharedMemory('shared_objects') as sm:
        sm.init_consumer()
        print('Receiver is ready.')

        done = False
        msgs_received = 0
        # sm.wait_for_messages()
        with wait_my_turn_when_has_messages(sm):
            mapped_flag_holder: List[bool] = sm.take_message()
        ...
        while not done:
            # sm.wait_for_messages(periodic_sleep_time=None)
            with wait_my_turn_when_has_messages(sm, periodic_sleep_time=None):
                while sm.has_messages():
                    some_binary, obj_offset = sm.take_message_2()
                    sm.destroy_obj(obj_offset)
                    msgs_received += 1
                    mapped_flag_holder[0] = True
                    done = mapped_flag_holder[1]
