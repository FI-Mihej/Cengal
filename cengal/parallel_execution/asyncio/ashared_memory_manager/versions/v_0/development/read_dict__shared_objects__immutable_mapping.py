                ...
                with wait_my_turn(sm, periodic_sleep_time=None):
                    ...
                    if processing_requested:
                        try:
                            ...
                            isolated_processing_time: float = cpu_clock() - start_time
                            control[ControlFields.isolated_processing_time] = isolated_processing_time
...
def sender(shared_memory_name: str):
    ...
    with SharedMemory(shared_memory_name, True, 1000 * 1024**2) as sm:
        ...
        with wait_my_turn(sm):
            ...
            with MeasureTimeTraceLine('Creating shared dict instances', control[ControlFields.players_num]) as mt:
                for player_id in range(mt.iterations):
                    ...
                    shared_players.append(fmapping(player_state))
        ...
        isolated_time_spent: float = control[ControlFields.isolated_processing_time]
        ...
        print(f'It took {isolated_time_spent} seconds to made {increments_num} isolated increments')
        increments_per_second: float = increments_num / isolated_time_spent
        if isolated_time_spent:
            print(f'There are {increments_per_second} isolated_increments/seconds')
