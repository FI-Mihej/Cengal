...
class PlayerFields(IntEnum):
    ...
    player_health = 2
    player_mana = 3
    player_position__x = 4
    player_position__y = 5
    player_position__z = 6
    player_rotation__x = 7
    player_rotation__y = 8
    player_rotation__z = 9
                    ...
                    if processing_requested:
                        try:
                            ...
                            for _, player_state in shared_dict_per_index.items():
                                player_state[PlayerFields.player_health] += 1
                                player_state[PlayerFields.player_mana] += 1
                                player_state[PlayerFields.player_position__x] += 1.0
                                player_state[PlayerFields.player_position__y] += 1.0
                                player_state[PlayerFields.player_position__z] += 1.0
                                player_state[PlayerFields.player_rotation__x] += 1.0
                                player_state[PlayerFields.player_rotation__y] += 1.0
                                player_state[PlayerFields.player_rotation__z] += 1.0
                                player_state[PlayerFields.player_inventory_size] += 1
                        finally:
    ...
    with SharedMemory(shared_memory_name, True, 1000 * 1024**2) as sm:
        ...
        with wait_my_turn(sm):
            control: List = intenum_dict_to_list({
                ...
                ControlFields.players_num: 3000,
                ControlFields.iterations_num: 1,
                ControlFields.sub_iterations_num: 5,
            })
            ...
            with MeasureTimeTraceLine('Creating shared dict instances', control[ControlFields.players_num]) as mt:
                for player_id in range(mt.iterations):
                    player_state: List = intenum_dict_to_list({
                        PlayerFields.player_id: player_id,
                        PlayerFields.player_name: f'Player_{player_id}',
                        PlayerFields.player_health: 100,
                        PlayerFields.player_mana: 100,
                        PlayerFields.player_position__x: 0.0,
                        PlayerFields.player_position__y: 0.0,
                        PlayerFields.player_position__z: 0.0,
                        PlayerFields.player_rotation__x: 0.0,
                        PlayerFields.player_rotation__y: 0.0,
                        PlayerFields.player_rotation__z: 0.0,
                        PlayerFields.player_inventory_size: 0,
                    })
        ...
        fields_per_player: int = len(shared_dict_per_index[0]) - 2
        ...
        print()
        print(list(shared_dict_per_index[0]))
