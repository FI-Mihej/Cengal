...
socket_path = "/tmp/asyncio_domain_socket"
    ...
    try:
        ...
        with MeasureTimeTraceLine('Writer', iterations=1000) as mt:
            for i in range(mt.iterations):
                writer.write(data)
                await writer.drain()  # Handle backpressure here

            writer.write_eof()  # Signal the end of data
    finally:
...
async def main():
    ...
    server_task.cancel()  # Then cancel the server
