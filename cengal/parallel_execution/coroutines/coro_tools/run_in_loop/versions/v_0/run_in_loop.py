...
from cengal.parallel_execution.coroutines.coro_tools.await_coro import RunSchedulerInAsyncioLoop, await_coro_fast
...
async def arun_in_loop(coro_worker: AnyWorker, *args, **kwargs):
    ...
    rs = RunSchedulerInAsyncioLoop(cs, 0.020, async_loop)
    rs.make_idle_when_possible = False
    rs.need_to_stop_when_possible = False
    rs.register()
    return await await_coro_fast(async_loop, cs, CoroType.auto, main, initial_setup_coro, coro_worker, *args, **kwargs)
