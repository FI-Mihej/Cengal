...
#cgo CFLAGS: "-IC:/Dev/Compilers/Python39/Include" -Wno-error -Wno-implicit-function-declaration -Wno-int-conversion
#cgo LDFLAGS: "-LC:/Dev/Compilers/Python39/libs" "-lpython39"
...
import (
	...
	greeter "cengal/_examples/ex_golang/versions/v_0/compilable/src"
)
...
// --- generated code for package: greeter below: ---
...
func greeter_Greet(name *C.char, goRun C.char) {
	_saved_thread := C.PyEval_SaveThread()
	defer C.PyEval_RestoreThread(_saved_thread)
	if boolPyToGo(goRun) {
		go greeter.Greet(C.GoString(name))
	} else {
		greeter.Greet(C.GoString(name))
	}
}

//export greeter_Greet2
func greeter_Greet2(name *C.char, goRun C.char) {
	_saved_thread := C.PyEval_SaveThread()
	defer C.PyEval_RestoreThread(_saved_thread)
	if boolPyToGo(goRun) {
		go greeter.Greet2(C.GoString(name))
	} else {
		greeter.Greet2(C.GoString(name))
	}
}
