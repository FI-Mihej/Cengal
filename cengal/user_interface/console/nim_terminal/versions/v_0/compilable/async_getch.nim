...
when defined(windows):
  ...
  proc prepareTryGetch*(ms: int32 = 1000): bool =
    return true

  proc tryGetch*(): tuple[num: int, ch: int] =
    result = (0, 0)
    return result

  proc winTryGetch*(ms: int32 = 1000): tuple[has_ch: bool, ch: int, has_vsc: bool, vsc: int] =
    ...
    if event.uChar != 0:
      result = (true, int(event.uChar), false, 0)
      return result
    else:
      result = (true, int(event.uChar), true, int(event.wVirtualScanCode))
      return result
  ...
  proc prepareTryGetch*(ms = 1000): bool =
    if kbhit(ms) > 0:
      return true
    else:
      return false
  ...
  proc tryGetch*(): tuple[num: int, ch: int] =
    var ch: char = char(0)
    var num: int = read(cint(STDIN_FILENO), ch.addr, 1)
    result = (num, int(ch))
    return result
