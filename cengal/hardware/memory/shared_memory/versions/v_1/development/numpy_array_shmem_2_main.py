...
shape = (10, 10, 3)
dtype = np.float64
original_np_arr = np.zeros(shape, dtype=dtype)
holder: bytes = original_np_arr.tobytes()
shared_memory_size = len(holder) * 2
...
def main():
    ...
    with wait_my_turn(creator):
        with test_run_time('Initialization Duration', 1, ignore_index=True):
            ...
            np_arr: np.ndarray = make_numpy_array_from_obj_offset(creator, holder_offset, shape, dtype)
