...
import pickle
import os
from data_containers import FrontTriggerableVariable, FrontTriggerableVariableType
from help_tools import load_old_class_to_the_new_one, BaseClassSettings
from ..simple_network import tcp_link
    ...
    def start(self):
        if self.subprocess_was_initiated:
            ...
            if self.settings.need_multithreading:
                self.subprocess = Thread(target=target, args=arguments)
            else:
                self.subprocess = Process(target=target, args=arguments)
