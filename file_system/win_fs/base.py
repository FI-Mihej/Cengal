...
from code_flow_control import is_ok_reader, IsOK_IntenalResult, ResultExistence
...
def verdictor(ok_status, result):
    if ok_status:
...
def verdict_exceptions_consolidator(context, block_results_criteria=None, close=False):
    with is_ok_reader(context, block_results_criteria, close):
        if not context:
            ...
            for block in reversed(bad_blocks):
                ...
                if type(block_result) != IsOK_IntenalResult:
