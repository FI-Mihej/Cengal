...
import shutil
from code_flow_control import ResultType, IsOK_ContextHolder, IsOkValRunner, CriteriaType, is_ok_reader, \
    IsOK_IntenalResult, ResultExistence
...
def copy(src_name, dst_name):
    cont = IsOK_ContextHolder('copy2', None, ResultType(CriteriaType.optional, set()))
    c_ret = IsOkValRunner(cont, lambda result: result != 0, verdictor)
    c_handle = IsOkValRunner(cont, lambda result: result.value != INVALID_HANDLE_VALUE_CTYPES, verdictor)

    c_ret()(ctypes.windll.kernel32.CopyFileW(WUPP + src_name, WUPP + dst_name, False))
    c_ret()(ctypes.windll.kernel32.SetFileAttributesW(WUPP + dst_name, FILE_ATTRIBUTE_NORMAL))
    destination_file_handle = c_handle('open Destination file')(
        create_file(WUPP + dst_name, GENERIC_WRITE, FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL))
    try:
