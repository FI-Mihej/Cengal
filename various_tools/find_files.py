...
def find_files(search_dir, file_name_filter_raw, interested_extensions):
    result_list = None
    ...
    for result in result_list:
        for filename in result[2]:
            if file_name_filter_raw.lower() in filename.lower():
                full_file_name = os.path.join(result[0], filename)
                print('>> {}'.format(full_file_name))
...
def main():
    ...
    last_search_string = global_config.get_property('find files', 'last search string') or ''
    last_dir = global_config.get_property('find files', 'last dir') or ''
    last_extensions = global_config.get_property('find files', 'last extensions') or ''

    # SEARCH REQUEST
    file_name_filter_raw = input('Enter file name (with * and ? filters), ("*") or leave empty to use last used '
                                 'value ({}): '.format(last_search_string))
    if len(file_name_filter_raw) == 0:
        file_name_filter_raw = last_search_string
    global_config.set_property('find files', 'last search string', file_name_filter_raw)
    if '*' == file_name_filter_raw.strip():
        file_name_filter_raw = ''
    ...
    global_config.set_property('find files', 'last dir', search_dir_list_raw)
    ...
    global_config.set_property('find files', 'last extensions', interested_extensions_raw)
    ...
    for search_dir in search_dir_list:
        find_files(search_dir, file_name_filter_raw, interested_extensions)
