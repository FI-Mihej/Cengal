...
def random_int(a, b):
    if a < -20:
        raise IsOK_BlockFailed('Too small!')
    result = randint(a, b)
    return result
...
def user_input(is_ok__context_holder=None):
    if is_ok__context_holder:
        ...
        str_len = len(uinput)
        if str_len < 10:
            raise IsOK_BlockFailed('Too short!')
        result = uinput
    ...
    with is_ok(function_context, 'main loop'):
        if function_context:
            for index in range(10):
                ...
                context = IsOK_ContextHolder('Test Holder', 'Some holder', ResultType(CriteriaType.forbidden, {'1 str'}),
                                             raise_exceptions=raise_exceptions)

                with is_ok(context, 0, None, ResultType(CriteriaType.any, None)):
                    if context:
                        result = 2 * 3
                        context.push_result(True, result)

                try:
                    with is_ok(context, '1 str', None, ResultType(CriteriaType.any, None)):
                        if context:
                            try:
                ...
                with is_ok_reader(context, ResultType(CriteriaType.forbidden, {'2 str'})):
                    if context:
                        print()
                        print('Everything is fine: result fit criteria of current reader block: '
...
RESULT: 625.0
RESULT: ISOK INTERNAL RESULT. BUILT-IN EXCEPTION: IsOK_BlockFailed (Too short!)
...
========================
    ...
    :return:
    '''
    cont = IsOK_ContextHolder('Test Holder', 'Some holder', ResultType(CriteriaType.any, None))
...
RESULT FOR "Test Holder": ISOK INTERNAL RESULT. BUILT-IN EXCEPTION: IsOK_BlockFailed (Too short!)
...
========================
    ...
    cr = IsOkUniRunner(globals(), cont, True)  # Simple-Mode - for any functions

    IsOkUniRunner(globals(), cont)('User Input').user_input()  # Normal-Mode - for specially prepared functions
    ...
    # Will print "625"
    IsOkReaderRunner(globals(), cont)().print_result(result_block_id)

    # Will print user input or error (exception info) with text "Too short!"
    crr = IsOkReaderRunner(globals(), cont)
...
(index(0), depth(0), ID(<function random_int at 0x000001D1D8A4EE18>), INFO(None), RESULT((False, ISOK INTERNAL RESULT. BUILT-IN EXCEPTION: IsOK_BlockFailed (Too small!)))),
...
========================
    ...
    # ============================
    # TEST 1:
    cont = IsOK_ContextHolder('Test Holder Number 1', 'Will be successful', ResultType(CriteriaType.optional, set()))
    cr = IsOkUniRunner(globals(), cont, True)  # Simple-Mode - for any functions
    ...
    # Will print "625"
    IsOkReaderRunner(globals(), cont)().print_result('Result')
    ...
    # ============================
    # TEST 2:
    cont = IsOK_ContextHolder('Test Holder Number 2', 'Will fail', ResultType(CriteriaType.optional, set()))
    cr = IsOkUniRunner(globals(), cont, True)  # Simple-Mode - for any functions
    ...
    cr('Result').int_square(  # Will not be computed
        cr().int_div(  # Will not be computed
            cr().random_int(-40, 0),  # -40 < -20. EXCEPTION IsOK_BlockFailed ("Too small!")
    ...
    # ============================
    # TEST 3:
    cont = IsOK_ContextHolder('Test Holder Number 3', 'Every function should return number >= 0. Will fail.',
                              ResultType(CriteriaType.optional, set()))
    cr = IsOkUniRunner(globals(), cont, True, lambda result: result >= 0)  # Simple-Mode - for any functions
    ...
    # ============================
    # TEST 4:
    cont = IsOK_ContextHolder('Test Holder Number 4', 'Every function should return even number. Will fail.',
                              ResultType(CriteriaType.optional, set()))
    cr = IsOkUniRunner(globals(), cont, True, lambda result: result % 2 == 0)  # Simple-Mode - for any functions
...
========================
    ...
    cr = IsOkUniCallRunner(cont, True)  # Simple-Mode - for any functions

    IsOkUniCallRunner(cont)('User Input')(user_input)  # Normal-Mode - for specially prepared functions
    ...
    # Will print "625"
    IsOkReaderCallRunner(cont)()(print_result, result_block_id)

    # Will print user input or error (exception info) with text "Too short!"
    crr = IsOkReaderCallRunner(cont)
    crr()(print_result, 'User Input')
...
========================
    ...
    # Will print "625"
    IsOkReaderCallRunner(cont)()(print_result, 'Result')
    ...
    cont = IsOK_ContextHolder('Test Holder Number 3', 'Every function should return number >= 0. Will fail.',
                              ResultType(CriteriaType.optional, set()))
    cr = IsOkUniCallRunner(cont, True, lambda result: result >= 0)  # Simple-Mode - for any functions
    ...
    cont = IsOK_ContextHolder('Test Holder Number 4', 'Every function should return even number. Will fail.',
                              ResultType(CriteriaType.optional, set()))
    cr = IsOkUniCallRunner(cont, True, lambda result: result % 2 == 0)  # Simple-Mode - for any functions
