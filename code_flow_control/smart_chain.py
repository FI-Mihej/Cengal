...
class IsOK_ContextHolder:
    def __init__(self, context_holder_id=None, context_holder_info=None, global_block_results_criteria=None,
        ...
        self._reserve_block_id_generator = IDGenerator.IDGenerator(IDGenerator.TypeOfGenerator.GUID_STRING)
...
def is_ok(context_holder, block_id, block_info=None, block_results_criteria=None, ignore_block_result_criteria=None):
    if block_id is None:
        block_id = (context_holder.self._reserve_block_id_generator.counter,
                    context_holder.self._reserve_block_id_generator.get_new_ID())
    ...
    try:
        ...
        if context_holder._save_debug_trace:
            result_string = 'ISOK INTERNAL RESULT. EXTERNAL EXCEPTION WITH TRACE:{} TRACE:{}'.format(error_str,
                                                                                                     trace_str)
        else:
            result_string = 'ISOK INTERNAL RESULT. EXTERNAL EXCEPTION: {} '.format(error_str)
        context_holder.push_result(False, IsOK_IntenalResult(
