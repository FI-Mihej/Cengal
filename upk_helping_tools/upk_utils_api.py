...
import shutil
from upk_helping_tools import upk_constants
...
def unpack_upk_files(upk_utils_dir, xcom_upk_dir, output_dir, copy_if_error=True, copy_already_unpacked_files=True):
    ...
    try:
        ...
        for filename in file_list_result[2]:
            ...
            # subprocess.call([os.path.join(upk_utils_dir, 'ExtractNameLists.exe'), full_file_name])
            # subprocess.call([os.path.join(upk_utils_dir, 'DeserializeAll.exe'), full_file_name], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=False)
            # subprocess.call([os.path.join(upk_utils_dir, 'ExtractNameLists.exe'), full_file_name], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=False)
    finally:
...
def export_from_upk_to_pseudo_code(upk_utils_dir, upk_file_full_name, object_full_name, output_mod_full_file_name=None):
    '''
    :param upk_utils_dir: 'C:\\Tools\\UPKUtils'
    :param upk_file_full_name: "C:\\Development\\XCOM Modding\\Work\\InChange\\XComGame.upk"
    :param object_full_name: 'XGAbility_Targeted.RollForHit'
    :param output_mod_full_file_name: "C:\\Development\\...\\pseudocode\\XGAbility_Targeted.RollForHit.code"
...
if __name__ == "__main__":
    export_from_upk_to_pseudo_code(r'C:\Tools\UPKUtils',
                                   r"C:\Development\XCOM Modding\Work\InChange\XComGame.upk",
                                   'XGAbility_Targeted.RollForHit',
                                   r"C:\Development\XCOM Modding\Work\pseudocode\XGAbility_Targeted.RollForHit.code")
